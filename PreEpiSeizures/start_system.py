...
def start_system(device_A, device_B, a_file, drift_log_file):
	...
	dig_Out = 0
	sync_param = {'flag_sync' : 0 , 'inittime' : time.time(), 'strtime': time.time(), 'sync_time' : time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime()), 'dig_Out' : dig_Out, 'close_file' : 0, 'diff': 1000, 'save_log': 1, 'count_a': 1000}
